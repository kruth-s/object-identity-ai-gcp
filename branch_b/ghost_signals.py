...
def _shadow_mask_score(bgr: np.ndarray) -> dict:
    # LAB separates luminance (L) from color
    lab = cv2.cvtColor(bgr, cv2.COLOR_BGR2LAB)
    L, _, _ = cv2.split(lab)

    # adaptive threshold highlights darker shadow-like areas
...
def _perspective_line_cues(bgr: np.ndarray) -> dict:
    ...
    if lines is None:
        return {"line_count": 0, "angle_hist": [0, 0, 0, 0]}
    ...
    # 4-bin histogram: roughly horizontal, vertical, diag+, diag-
...
def ghost_signal_features_from_bytes(image_bytes: bytes) -> dict:
    ...
    if bgr is None:
        return {"error": "Invalid image for ghost signals"}
    ...
    vector = [
        shadow["shadow_ratio"],
        float(perspective["line_count"]),
        *perspective["angle_hist"],
        intensity["gradient_energy"],
    ]
